...
pub fn render_issue_list(
    ...
    if issues.is_empty() {
        let empty_msg = vec![
            Line::from(""),
            Line::from(vec![
                Span::styled("  ✔ Keine offenen Probleme gefunden! ", Style::default().fg(Theme::EMERALD).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("  Ihr System befindet sich in einem einwandfreien Zustand.", Style::default().fg(Theme::TEXT_WHITE)),
            ]),
            Line::from(vec![
                ...
                Span::styled(", um einen neuen System-Health-Scan zu starten.", Style::default().fg(Theme::MUTED)),
            ]),
        ];
    ...
    let items: Vec<ListItem> = issues
        ...
        .map(|(idx, issue)| {
            ...
            let line = Line::from(vec![
                Span::styled(format!(" {} ", check_box), Style::default().fg(check_color).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{} ", sev_str), Style::default().fg(sev_color)),
                Span::styled(
                    issue.title.clone(),
                    if is_current {
                        Style::default().fg(Theme::BG_DEEP).bg(Theme::CYAN).add_modifier(Modifier::BOLD)
                    } else {
    ...
    let issue_list = List::new(items).block(
        Block::default()
            ...
            .title(format!(" GEFUNDENE PROBLEME [{}] – [Space] An-/Abwählen ", issues.len())),
    );
    ...
    if let Some(issue) = issues.get(selected_index) {
        ...
        let mut detail_lines = vec![
            Line::from(vec![
                ...
                Span::styled(issue.title.clone(), Style::default().fg(Theme::TEXT_WHITE).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(sev_badge, Style::default().fg(sev_color).add_modifier(Modifier::BOLD)),
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled(" Beschreibung:", Style::default().fg(Theme::CYAN).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
                Span::styled(format!("  {}", issue.description), Style::default().fg(Theme::TEXT_WHITE)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled(" Technische Diagnose / Fund:", Style::default().fg(Theme::CYAN).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
                Span::styled(format!("  {}", issue.technical_details), Style::default().fg(Theme::MUTED)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled(" Empfohlene Reparatur (Auto-Fix):", Style::default().fg(Theme::EMERALD).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
                Span::styled(format!("  👉 {}", issue.recommended_fix), Style::default().fg(Theme::TEXT_WHITE)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled(" Reparatur-Schritte:", Style::default().fg(Theme::MUTED).add_modifier(Modifier::BOLD)),
            ]),
        ];

        for (step_idx, step) in issue.fix_steps.iter().enumerate() {
            detail_lines.push(Line::from(vec![
                Span::styled(format!("   {}. ", step_idx + 1), Style::default().fg(Theme::CYAN)),
        ...
        detail_lines.push(Line::from(vec![
            Span::styled(" 🛡 VSS Wiederherstellungspunkt wird vor Reparatur automatisch erstellt.", Style::default().fg(Theme::EMERALD).add_modifier(Modifier::ITALIC)),
        ]));
