...
| **Event Log & Crash Analysis** | WHEA hardware error architecture logs, `%SystemRoot%\Minidump` BSOD crash dumps, the result of the last Windows Memory Diagnostic (90 days) | Crash dump cleanup; WHEA faults and a failed memory test are advice, since no software repair fixes hardware |
