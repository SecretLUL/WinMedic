...
- **Scan & Repair** — the top of the page says what state the PC is in and offers the one step that makes sense next: *Scan now* on a machine that has never been checked, *Repair* once there is something to repair, *Cancel* while something runs. What the rest of the page shows depends on the mode:
  - **Easy mode** (what WinMedic opens in) lists the findings in plain words, in four groups: what WinMedic repairs, what waits for a restart, what it leaves for you to decide, and what is already repaired. The *Repair* button repairs the first group — the findings the checks recommend. Once only restarts are left, the page asks for one and offers *Restart now*.
