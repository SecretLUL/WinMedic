...
394 tests pass; `cargo fmt` and `cargo clippy --all-targets -- -D warnings` are clean against `x86_64-pc-windows-msvc`. CI additionally enforces the MSRV (Rust 1.88) and audits dependencies on every pull request.
